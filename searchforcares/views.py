...
class SearchMotif(ObjectViewMixin, ListView):
  ...
  def fill_color(self,fragment):
    ...
    for factor in vcc_list:
        ...
        for match in matches:
            ...
            overlap = any(start < colored_end and end > colored_start for colored_start, colored_end in colored_positions)
            if not overlap:
  ...
  def get_context_data(self, **kwargs):
    ...
    if self.request.user.is_authenticated :
      fdd = History_search_care.objects.create(F_r=context['fragment'],
      ...
      query = self.request.GET.get('sequencetosubmit', '')  # Lấy thông tin tìm kiếm từ request
      user = self.request.user if self.request.user.is_authenticated else None  # Lấy thông tin người dùng
      new_history = History.objects.create(user=user, query=query,Belong_history_search_care=fdd)
